...
export default function Dashboard() {
  ...
  return (
    ...
    >
      <DemoHint persona="System overview" hint="Submit a pain point on the Map — it appears in the live feed below. Stats update when internal leads advance startups." />
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05, duration: 0.35 }}
        ...
        {kpis.map(k => (
          <div key={k.label} style={{ background: 'var(--surface)', border: `1px solid ${k.primary ? 'rgba(200,240,0,0.25)' : 'var(--border)'}`, borderTop: `2px solid ${k.primary ? 'var(--lime)' : 'var(--border)'}`, borderRadius: 'var(--radius)', padding: '14px 16px' }}>
            <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-faint)', display: 'block', marginBottom: '8px' }}>{k.label}</span>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.35 }}
              ...
              <div style={{ borderRight: '1px solid var(--border)', padding: '14px 10px' }}>
                ...
                {navItems.map(item => (
                  <div key={item.label}
                    ...
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 8px', borderRadius: 'var(--radius-sm)', marginBottom: '1px', cursor: 'pointer', transition: 'background 0.15s', background: item.active ? 'rgba(200,240,0,0.07)' : 'transparent', border: item.active ? '1px solid rgba(200,240,0,0.15)' : '1px solid transparent' }}>
        ...
        <div style={{ gridColumn: 'span 4' }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', height: '100%' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 14px', borderBottom: '1px solid var(--border)', background: 'var(--surface-2)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '8px', color: 'var(--lime)', letterSpacing: '0.1em' }}>live</span>
              </div>
            </div>
            {painFeed.map((item, i) => (
              <div key={item.text} style={{ padding: '10px 14px', borderBottom: i < painFeed.length - 1 ? '1px solid var(--border)' : 'none', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.35 }}
        ...
        {[
          ...
          <motion.div key={p.num} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 + i * 0.05, duration: 0.35 }}
            ...
            <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', letterSpacing: '0.14em', textTransform: 'uppercase', color: p.color, display: 'block', marginBottom: '8px' }}>{p.num} · {p.name}</span>
