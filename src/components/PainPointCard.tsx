...
const statusStyles: Record<string, { color: string; bg: string; label: string }> = {
  ...
  matched: { color: 'var(--lime)', bg: 'rgba(200,240,0,0.10)', label: 'Matched' },
    ...
    <div className="card" style={{ padding: '16px', transition: 'all 0.15s' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '8px' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontFamily: 'IBM Plex Sans', fontWeight: 600, fontSize: '13px', color: 'var(--text)', marginTop: '3px', lineHeight: '1.4' }}>
            {painPoint.title}
          </div>
        </div>
        <span style={{
      ...
      <div style={{ fontFamily: 'IBM Plex Sans', fontSize: '12px', color: 'var(--text-muted)', lineHeight: '1.5', marginBottom: '10px' }}>
        {painPoint.description.length > 140 ? painPoint.description.slice(0, 140) + '…' : painPoint.description}
      </div>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
        <div style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', color: 'var(--text-faint)', letterSpacing: '0.08em' }}>
        ...
        {linkedApp && (
          <div style={{
            ...
            <span style={{ fontFamily: 'IBM Plex Mono', fontSize: '9px', color: 'var(--lime)', letterSpacing: '0.08em' }}>
